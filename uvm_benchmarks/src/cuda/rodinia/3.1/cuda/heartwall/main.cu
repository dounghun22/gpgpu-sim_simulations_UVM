...
int main(int argc, char *argv []){
	...
	//params_common_change* common_change;
	cudaMallocManaged(&common_change, sizeof(params_common_change));

	//params_common* common;
	cudaMallocManaged(&common, sizeof(params_common));

	//params_unique* unique;								// cannot determine size dynamically so choose more than usually needed
	cudaMallocManaged(&unique[ALL_POINTS], sizeof(params_unique)*ALL_POINTS);
