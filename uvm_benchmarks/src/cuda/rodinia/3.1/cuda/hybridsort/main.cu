			  ...
			  float *resultList, int numElements)
{
	...
	{
		cudaMallocManaged((void**) &d_input, mem_size);
		cudaMallocManaged((void**) &d_output, mem_size);
		memcpy((void *) d_input, (void *)origList, numElements * sizeof(float));
	...
	memcpy((void *) resultList, (void *)mergeresult, numElements * sizeof(float));
